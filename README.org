...
- ~:indexed-fields~ - Explicitly choose which fields participate in full-text indexing. If omitted, all fields are indexed.
...
- ~:keyword-fields~ - Fields that should be treated verbatim rather than tokenized.
- ~:suggest-fields~ - Fields that are treated specially during indexing, allowing Lucene to create internal structures for quick prefix matching. This can be a list of field names, or ~[field-name weight]~ pairs.
- ~:context-fn~ - A keyword or function used to derive a list of contexts for /suggest/ fields, allowing suggestions to be filtered at query time.
...
Weighted suggestions are also supported:
#+BEGIN_SRC clojure :tangle no :results none
(lucene/index! album-index common/album-data
               {:stored-fields  [:Number :Year :Album :Artist :Genre :Subgenre]
                :indexed-fields [:Album :Artist :Genre :Subgenre :Year]
                :keyword-fields [:Year :Genre :Subgenre]
                :suggest-fields [[:Album 5] :Artist]
                :context-fn     (fn [doc] (:Genre doc))})
#+END_SRC
...
For convenience, /lucene-clj/ has a function that can be used to convert the Lucene /Document/ into a Clojure map. It supports both ~:multi-fields~ and ~:fields-to-keep~ to shape the result. But beyond basic use-cases, supply your own.
#+BEGIN_SRC clojure :results output code :tangle dev/dev.clj :exports both
(lucene/search album-index {:Year "1977"}
               {:results-per-page 2
...
When the query form is a plain string, pass ~:field-name~. Strings containing spaces are treated as phrase queries.
...
From above, the fields ~Album~ and ~Artist~ have been marked to be indexed in a way so that we can ask for prefix-based suggestions. During indexing, ~:suggest-fields~ can be a simple field name or a weighted ~[field-name weight]~ pair.
