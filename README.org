...
Here's the background, used in the examples below as well as the tests. Hence the /tests-common/ namespace. Note that for the albums dataset, some of the columns - notably /genre/ and /sub-genre/, are comma-separated values themselves, and hence they are pre-processed before feeding to /lucene-clj/.
